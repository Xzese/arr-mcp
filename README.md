...
### Search media already in the library

Use an exact library ID to start a native Radarr or Sonarr search:
...
sonarr_episodes(operation="search", series_id=42, season_number=2)
```

Each operation checks that the requested movie, series, episode, or season
exists before submitting a command. The response confirms that the search
command was submitted; it does not mean a download has completed. Use
`operation="lookup"` to find metadata for media that is not in the library.
